...
#' @param sloop_smooth logical When sharp slops exist, set this parameter to TRUE to perform a post-processing which will reduced errors.
#' @param class_threshold scalar The distance to the simulated cloth to classify point cloud into ground and non-ground. The default is 0.5.
#' @param cloth_resolution scalar The distance between paticles in cloth. This is usually set to the average distance of the points in the point cloud. The default value is 0.5.
#' @param rigidness integer The rididness of the cloth. 1 stands for very soft cloth (to fit rugged terrain), 2 stands for medium cloth and 3 stands for hard cloth (for flat terrain). The default is 1.
#' @param interations integer Maximum iteration for simulating cloth. The default value is 500. Usually, users do not need to change this.
#' @param time_step scalar. Time step when simulating the cloth under the gravity. The default value is 0.65. Usually, Do not change this value.It is suitable for most cases.
#'
...
CSF <- function(cloud, sloop_smooth = FALSE, class_threshold = 0.5, cloth_resolution = 0.5, rigidness = 1L, interations = 500L, time_step = 0.65)
{
    ...
    if(!is.numeric(interations) || interations <= 0)
      stop("'interations' must be a integer larger than 0")
